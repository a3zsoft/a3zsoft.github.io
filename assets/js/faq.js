...
const faqsData = [
    ...
    { q: 'Can I change Gro in my language and currency?', a: 'Yes! During setup or in Settings, choose your preferred language and set your primary currency. All transactions and calculations will use this currency for consistency.' },
    ...
    { q: 'What languages does Gro support?', a: 'Gro supports a wide range of languages to cater to a global audience. Currently, these include: English (en_US), Spanish (es_ES), French (fr_FR), Portuguese (pt_BR), Russian (ru_RU), German (de_DE), Japanese (ja_JP), Chinese (zh_CN), Hindi (hi_IN), Bengali (bn_BD), Tamil (ta_IN), Punjabi (pa_IN), Marathi (mr_IN), Telugu (te_IN), Vietnamese (vi_VN), Javanese (jv_ID), Turkish (tr_TR), Korean (ko_KR), Italian (it_IT), Thai (th_TH), Indonesian (id_ID), Cantonese (yue_HK), Malay (ms_MY), and Hausa (ha_NG).' },
    { q: 'What currencies does Gro support?', a: 'Gro supports a wide range of currencies to cater to users worldwide. Currently, these include: United States Dollar (USD), Euro (EUR), Japanese Yen (JPY), British Pound (GBP), Chinese Yuan Renminbi (CNY), Australian Dollar (AUD), Canadian Dollar (CAD), Switzerland Franc (CHF), Hong Kong Dollar (HKD), Singapore Dollar (SGD), Swedish Krona (SEK), South Korea Won (KRW), Norway Krone (NOK), New Zealand Dollar (NZD), Indian Rupee (INR), Mexican Peso (MXN), Taiwan New Dollar (TWD), South Africa Rand (ZAR), Brazilian Real (BRL), Denmark Krone (DKK), Poland Zloty (PLN), Thailand Baht (THB), Israel Shekel (ILS), Indonesia Rupiah (IDR), Czech Koruna (CZK), Emirati Dirham (AED), Turkish Lira (TRY), Hungary Forint (HUF), Chile Peso (CLP), Saudi Arabia Riyal (SAR), Philippines Peso (PHP), Malaysia Ringgit (MYR), Colombia Peso (COP), Russia Ruble (RUB), Romania Leu (RON), Peru Sol (PEN), UEMOA CFA franc (XOF), Central African CFA franc (XAF), Egypt Pound (EGP), Argentine Peso (ARS), Pakistan Rupee (PKR), Vietnamese Dong (VND), Nigeria Naira (NGN), Ukraine Hryvnia (UAH), Algerian dinar (DZD), Moroccan Dirham (MAD), Kazakhstan Tenge (KZT), Kuwaiti Dinar (KWD), Bahraini Dinar (BHD), Oman Rial (OMR), Jordanian dinar (JOD), Iceland Krona (ISK), Uruguay Peso (UYU), Tunisian dinar (TND), Guatemalan quetzal (GTQ), Costa Rica Colon (CRC), Dominican Republic Peso (DOP), Nepal Rupee (NPR), Sri Lanka Rupee (LKR), Ghana Cedi (GHS), Ethiopian Birr (ETB), Azerbaijani Manat (AZN), Botswanan Pula (BWP), Jamaica Dollar (JMD), Trinidad and Tobago Dollar (TTD), Congolese franc (CDF), Belarussian Ruble (BYN), Ugandan Shilling (UGX), Cambodian riel (KHR), Kyrgyzstani som (KGS), Mauritius Rupee (MUR), Paraguay Guarani (PYG), Somalia Shilling (SOS), Syrian Lira (SYP), Tanzanian Shilling (TZS), Yemen Rial (YER), Zambian kwacha (ZMW), Bhutanese Ngultrum (BTN), Bolivia Bolíviano (BOB), Myanmar Kyat (MMK), Malagasy Ariary (MGA), Rwandan franc (RWF), Seychellois rupee (SCR), Brunei Darussalam Dollar (BND), Liberia Dollar (LRD), Belize Dollar (BZD), Guyana Dollar (GYD), Bermuda Dollar (BMD), Barbados Dollar (BBD), Tongan Pa\'anga (TOP), Maldivian Rufiyaa (MVR), Mongolia Tughrik (MNT), Nicaragua Cordoba (NIO), Papua New Guinean Kina (PGK), Suriname Dollar (SRD), Burundian Franc (BIF), Gambian dalasi (GMD), Haitian gourde (HTG), Laos Kip (LAK), Malawian Kwacha (MWK), Macedonia Denar (MKD), Moldovan Leu (MDL), Mozambique Metical (MZN), Afghanistan Afghani (AFN), Albanian Lek (ALL), Angolan Kwanza (AOA), Armenian Dram (AMD), Bosnia and Herzegovina convertible mark (BAM), Cuban Peso (CUP), Venezuelan Bolívar (VEF), Zimbabwean Dollar (ZWL). You can select your preferred currency during setup or in Settings > Currency.' },
